...
%>
 	<script type="text/javascript">
 		console.info("****************************************************************");
 		console.info("这里是addPrice页面的值");
  		//获取add.jsp页面传递的参数paramObj
 		var paramObj=$("#paramObj").val();
 		console.info("paramObj==="+paramObj);
 		//paramObj==={"skuValue":"aaa","oneParam":[{"regionCode":"566","regionPrice":"9"},{"regionCode":"567","regionPrice":"8"},{"regionCode":"568","regionPrice":"7"}]}
		//把json转换为js对象
 		var oldParam = $.parseJSON(paramObj);//对象 封装单品规格代码oldSkuValue和数组oldRegionParam
 		console.info("转换为js对象之后  oldParam==="+oldParam);
		var oldSkuValue = oldParam['skuValue']; //属性 保存的单品规格代码
		var oldRegionParam = oldParam['oneParam'];//数组 存放对象该对象封装区域代码oldRegionId和区域价格oldRegionPrice
		var oldIndex = oldParam['index'];
		var tempRegionParam = [];
 		//树节点展示数据		
		var datas = [];
		...
		function dochange(id, pid, parentValue){
			...
			{	
				var findflag = "false";
				if (oldRegionParam == null)
				{
					oldRegionParam = [];
				}
				for (var j=0; j<oldRegionParam.length; j++)
				{
					if (oldRegionParam[j].regionCode == tempRegionParam[i].regionCode)
					{
						oldRegionParam.splice(j,1);
						oldRegionParam.push(tempRegionParam[i]);
						findflag = "true";
						break;
					}
				}
			...
			for (var i=0; i<datas.length; i++){
				if (datas[i].pid == id){
					...
					if (value == 0|| typeof(value) == "undefined"){
						if(oldRegionParam !=null && oldRegionParam.length>0){
							for (var j=0; j<oldRegionParam.length; j++){
								if (oldRegionParam[j].regionCode == datas[i].id){
									value = oldRegionParam[j].regionPrice;
									break;
								}
							}
						}
					...
					if(value!=0){
						var newObj1 = {"regionCode":datas[i].id, "regionPrice":value};
						result.push(newObj1);
					}
		...
		function dosave(){
			...
			for (var i=0; i<datas.length; i++){
				if (datas[i].pid == -1){
					var value = $("#t_"+datas[i].id).val();
					if(value != 0){
						var newObj = {"regionCode":datas[i].id, "regionPrice":value};
						result.push(newObj);
					}
					getValue(datas[i].id, value);
				}
			...
			//保存所有参数			
			allresult = {index:oldIndex, skuValue:oldSkuValue, result:result};
			//传递所有参数
