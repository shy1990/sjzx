...
<script type="text/javascript">
	...
	$(function(){
		...
		integralCode.blur(function(){
			//取得文本框的值
	 		...
	 		//发送异步请求
	 		$.ajax({
				url : "integral/integralAction!gainIntegralGoodsByIntegralCode.action",
				type : "POST",
				dataType : "JSON",
				data : "integralCode="+integralCode,
				success : function(j) {
					if(j.success){
						//如果返回值为true，则校验通过
		...
		//取得文本框的值
	 		...
	 		//发送异步请求
	 		$.ajax({
				url : "integral/integralAction!gainIntegralGoodsBySpecCode.action",
				type : "POST",
				dataType : "JSON",
	...
	function makerUploadForAdd(chunk) {
		UploaderForAdd(chunk, function(files, thumbPaths) {
			if (files && files.length > 0) {
				var admin_integral_add_thumbPic =document.getElementById('admin_integral_add_thumbPic');
				...
				if(thumb.length>1){
					...
					for ( var i = 0; i < thumb.length; i++) {
						...
						img.ondblclick= function(){
							integral_add_dropDom(this);
						};
						admin_integral_add_thumbPic.appendChild(img);
						var picSrc = [];
						var temp = $('#admin_integral_add_thumbPic img');
	 ...
	 * 双击图片可进行图片删除
	 */	
	function integral_add_dropDom(obj){
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:true,split:false">
		<form id="admin_integral_addForm" method="post">
			<table  id="admin_integral_datagrid" class="tableForm" style="margin-left: 70px;margin-top: 15px">
				<tr>	
					<th>积分商品名称：</th>
					<td><input id="name" name="name" class="easyui-validatebox" data-options="required:true" style="width:160px" placeholder="请输入配件名称"/></td>
				</tr>
				<tr>	
					<th>商品代码：</th>
					<td>
					<input id="integralCode" name="integralCode" class="easyui-validatebox" data-options="required:true" style="width:160px" placeholder="请输入商品代码"/>
				...
				<tr>	
					<th>规格代码：</th>
					<td>
					<input id="specCode" name="specCode" class="easyui-validatebox" data-options="required:true" style="width:160px" placeholder="请输入规格代码"/>
					<span id="specCodeMsg"></span>
					</td>
				</tr>
				<tr>	
					<th>价格：</th>
					<td><input id="price" name="price" style="width:160px" class="easyui-validatebox" data-options="required:true" placeholder="请输入配件价格" onblur="validPrice();"/><span id="priceMsg"></span></td>
				</tr>
				<tr>
					<th>换取所需积分：</th>
					<td><input id="integral" name="integral" style="width:160px" class="easyui-validatebox" data-options="required:true" placeholder="请输入换取所需积分" onblur="validIntegral();"/><span id="integralMsg"></span></td>	
				</tr>	
				<tr>
					<th>库存：</th>
					<td><input id="stock" name="stock" style="width:160px" class="easyui-validatebox" data-options="required:true" placeholder="请输入库存" onblur="validStock();"/><span id="stockMsg"></span></td>
				</tr>
				<tr>
					<th>是否上架：</th>
					<td>
				...
				<tr>
					...
					<td>
						...
						<span id="admin_integral_add_thumbPic"> </span>	
